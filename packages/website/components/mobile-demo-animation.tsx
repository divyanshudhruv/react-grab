...
const HIDDEN_BOX: BoxState = { visible: false, x: 0, y: 0, width: 0, height: 0 };
...
const HIDDEN_LABEL: LabelState = { visible: false, x: 0, y: 0, componentName: "", tagName: "" };
  ...
  <svg width="32" height="32" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
    <defs>
      ...
      <mask id="busye" fill="#fff">
        <path d="m1 23c0 4.971 4.03 9 9 9 4.97 0 9-4.029 9-9 0-4.971-4.03-9-9-9-4.97 0-9 4.029-9 9z" fill="#fff" fillRule="evenodd"/>
      </mask>
    </defs>
...
export const MobileDemoAnimation = (): ReactElement => {
  ...
  const metricCardPosition = useRef<Position>({ x: 0, y: 0, width: 0, height: 0 });
  const metricValuePosition = useRef<Position>({ x: 0, y: 0, width: 0, height: 0 });
  const exportButtonPosition = useRef<Position>({ x: 0, y: 0, width: 0, height: 0 });
  ...
  const measureElementPositions = (): void => {
    ...
    const measureRelativePosition = (element: HTMLElement | null, positionRef: React.MutableRefObject<Position>): void => {
      if (!element) return;
  ...
  useEffect(() => {
    ...
    const displaySelectionLabel = (x: number, y: number, componentName: string, tagName: string): void => {
    ...
    const animateDragSelection = async (startX: number, startY: number, endX: number, endY: number): Promise<void> => {
      const dragSteps = 14;
    ...
    const executeAnimationSequence = async (): Promise<void> => {
      ...
      displaySelectionLabel(buttonCenter.x, buttonPos.y + buttonPos.height + 10, "ExportBtn", "button");
      await wait(600);
      if (isCancelled) return;
      ...
      displaySelectionLabel(cardPos.x + cardPos.width / 2, cardPos.y - 10, "MetricCard", "div");
      setLabelMode("grabbing");
      await wait(500);
      if (isCancelled) return;
      ...
      displaySelectionLabel(valuePos.x + valuePos.width + 10, valueCenter.y - 10, "StatValue", "span");
      await wait(600);
      if (isCancelled) return;
    ...
    const initializeAnimationLoop = (): void => {
      ...
      animationInterval = setInterval(executeAnimationSequence, ANIMATION_LOOP_INTERVAL_MS);
    };
  ...
  return (
    <div className="mt-3">
      ...
      <div className="overflow-hidden rounded-xl border border-white/10 bg-neutral-900 shadow-lg shadow-black/20">
        <div ref={containerRef} className="relative p-4">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <div className="text-[13px] font-semibold text-white">Overview</div>
          ...
          <div className="mb-4 grid grid-cols-3 gap-2.5">
            <div ref={metricCardRef} className="rounded-lg border border-white/10 bg-white/5 p-2.5">
              <div className="mb-1 text-[10px] font-medium text-white/50">Revenue</div>
              <div ref={metricValueRef} className="text-[18px] font-semibold tabular-nums text-white">
                $12.4k
              </div>
            ...
            <div className="rounded-lg border border-white/10 bg-white/5 p-2.5">
              <div className="mb-1 text-[10px] font-medium text-white/50">Users</div>
              <div className="text-[18px] font-semibold tabular-nums text-white">2,847</div>
              <div className="mt-1 text-[10px] text-white/50">+8.2%</div>
            </div>

            <div className="rounded-lg border border-white/10 bg-white/5 p-2.5">
              <div className="mb-1 text-[10px] font-medium text-white/50">Orders</div>
              <div className="text-[18px] font-semibold tabular-nums text-white">384</div>
          ...
          <div className="rounded-lg border border-white/10">
            <div className="border-b border-white/10 px-3 py-2">
              <div className="text-[11px] font-medium text-white/70">Recent Activity</div>
            </div>
            <div className="divide-y divide-white/10">
              {[
                ...
                <div key={activity.label} className="flex items-center justify-between px-3 py-2">
                  <div className="flex items-center gap-2">
                    <div className="h-5 w-5 rounded-full bg-white/10" />
                    <span className="text-[11px] text-white/70">{activity.label}</span>
                  </div>
          ...
          >
            <div className="absolute left-0 right-0 h-px bg-[#d239c0]" style={{ top: cursorPos.y }} />
            <div className="absolute bottom-0 top-0 w-px bg-[#d239c0]" style={{ left: cursorPos.x }} />
          </div>
          ...
          <div
            ...
            style={{ left: cursorPos.x - CURSOR_OFFSET_PX, top: cursorPos.y - CURSOR_OFFSET_PX }}
          >
          ...
          >
            {labelMode === "selecting" && (
              <>
                <span className="text-[13px] leading-4 font-medium text-black">{label.componentName}</span>
                <span className="text-[13px] leading-4 font-medium text-black/50">.{label.tagName}</span>
              </>
            )}
            {labelMode === "grabbing" && (
              <>
      ...
      <p className="mt-1.5 text-sm text-white/30">This website is best viewed on desktop</p>
    </div>
  );
};
